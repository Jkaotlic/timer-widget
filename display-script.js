...
class DisplayTimer {
    constructor() {
        ...
        this.cache = {
            ...
            lastStatus: null,
            lastProgress: null,
            // Полоса срочности в ключе перерисовки — рядом со значением
            // прогресса, а не вместо него. Почему одного значения мало,
            // разобрано у самих ворот в updateDisplay().
            lastBand: null,
    ...
    updateDisplay() {
        ...
        //
        // Считается ОДИН раз на тик: ту же величину читают ворота перерисовки
        // прогресса ниже, и второй вызов _colorBand() был бы вторым ответом на
        // один вопрос.
        ...
        // Прогресс перерисовывается, когда изменилось ЗНАЧЕНИЕ прогресса ИЛИ
        // ПОЛОСА срочности.
        //
        // Одного значения мало не по недосмотру, а в принципе:
        // calculateProgressValue() возвращает КОНСТАНТУ 0, когда геройского
        // тотала нет (режимы «Текущее время» и «До начала» — всегда, «До
        // конца» — когда конец не позже начала). В таких режимах значение не
        // меняется НИКОГДА, а полоса меняется: она приходит из
        // timerColorBand(), которая отдаёт overtime по одному знаку секунд, до
        // всякого обращения к тоталу. По значению отличить «полоса сменилась»
        // от «ничего не произошло» в этих режимах невозможно — сравнение с
        // любым порогом всегда даёт «не изменилось».
        //
        // Замерено 08.09.2026: без полосы в ключе updateProgress() звалась
        // ровно один раз, на входе в режим, и дальше краска замерзала —
        // отметка «до начала» проходила, а круг оставался цветом темы; отметка
        // уезжала обратно в будущее, а <body> и timeDisplay оставались
        // красными, хотя цифры уже становились обычными. Два стиля на одном
        // экране расходились в утверждении об одном и том же состоянии.
        const progress = this.calculateProgressValue();
        if (this.cache.lastProgress !== progress || this.cache.lastBand !== band) {
            this.updateProgress();
            this.cache.lastProgress = progress;
            this.cache.lastBand = band;
        }
    ...
    updateProgress() {
        ...
        // Тотал берётся ОДИН раз: он и ворота геометрии, и признак «нижней
        // полосы тут быть не должно».
        //
        // Полоса по нижнему краю выражает ДОЛЮ пройденного. Без тотала доли не
        // существует, и любая её ширина — утверждение о величине, которой нет:
        // в режиме «до начала» после прошедшей отметки body.overtime тянул за
        // собой `width: 100% !important` (display.css) и рисовал залу полную
        // красную шкалу «мероприятие пройдено на 100 %» для величины, у
        // которой hero-modes.js явно объявил отсутствие тотала.
        //
        // Признак — САМ ТОТАЛ, а не имя режима: у «до конца» с концом не позже
        // начала тотал тоже нулевой, и правило по имени режима эту дырку
        // оставило бы. Гасит КЛАСС на <body> (цвет и состояние в этом проекте
        // принадлежат каскаду), и он же снимает конфликт с `!important` — не
        // перебивая его вторым `!important`, а убирая элемент из показа.
        //
        // Прячется заливка, а не сама шкала: пустой серый жёлоб виден и
        // сегодня на дисплее без пресета (timer, totalSeconds === 0), и режим
        // «таймер» обязан выглядеть ровно как до 08.09.2026.
        const heroTotal = this._heroTotal();
        document.body.classList.toggle('no-hero-total', heroTotal <= 0);
        ...
        if (heroTotal > 0) {
            ...
            //
            // Обнуление ширины остаётся и после того, как заливку стал прятать
            // класс `no-hero-total`: класс отвечает за «не показывать», а эта
            // строка — за «не хранить прежнюю ширину». Иначе возврат в режим с
            // тоталом на один кадр показал бы полосу от прошлого доклада.
            if (this.displayProgressFill) {
