 ...
 *    Два доклада по 2 секунды при ставке «1000 за 3» стоят 1000 (4 с — одна
 *    полная ступень), а сложение цен строк дало бы 0 (по 2 с — ни одной).
...
/**
 * Начало ТЕКСТА, которое табличный редактор примет за формулу (SEC-09,
 * CWE-1236): = + - @, их полноширинные двойники, табуляция и CR.
 */
 ...
 *
 * Текст, похожий на формулу, получает апостроф: название мероприятия вводит
 * один человек, а файл открывает другой, и «=HYPERLINK(…)» в шапке — это
 * ссылка, по которой бухгалтер щёлкнет. Апостроф трогает только ТЕКСТ: число
 * (номер строки) формулой не бывает, и минус у числа — знак, а не команда.
 */
function csvCell(value) {
    const isNumber = typeof value === 'number' && Number.isFinite(value);
    let text = value === null || value === undefined ? '' : String(value);
    const formula = !isNumber && FORMULA_START.test(text);
