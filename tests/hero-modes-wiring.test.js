...
test('«До завершения» гасится в режиме «до конца» — тем же положительным селектором', () => {
    ...
    // Плашка считает ТОЙ ЖЕ формулой, что герой «До конца», — с началом
    // мероприятия, иначе конец после полуночи читался как прошедший (кламп по
    // одной отметке давал 00:00:00 в 23:30 при конце 00:10). Кламп в ноль у
    // плашки остался, но внутри общей функции.
    const code = SRC;
    const OLD = /secondsUntilClock\s*\(/;
    // Проверка проверки: регулярка отсутствия ловит старый вызов.
    assert.ok(OLD.test('window.RendererShared.secondsUntilClock(nowSeconds, this.endTime)'));
    assert.ok(/secondsUntilEventEnd\(nowSeconds, this\.eventTime, this\.endTime\)/.test(code),
        'плашка «До завершения» считает не через secondsUntilEventEnd(начало, конец)');
    assert.ok(!OLD.test(code),
        'в display-script.js снова зовут secondsUntilClock — полночь опять станет нулём');
});
