...
test('сохранённая позиция восстанавливается только на реально подключённый монитор', () => {
    ...
    // Монитор сохранённой точки ищется среди РЕАЛЬНО подключённых, с откатом на
...
test('дисплей не пишет цвет полосы инлайном ни в одном стиле', () => {
    ...
    //
    // Форма записи тоже несущая: переменная СТАВИТСЯ И СНИМАЕТСЯ одним
    // помощником. Односторонняя запись («поставить, если цвет пришёл») делает
    // цвет несбрасываемым — «Сбросить всё» шлёт объект БЕЗ полей, окно ничего
    // не применяет и продолжает краситься прошлым значением. Замер 20.08.2026:
    // после сброса дисплей оставался неоновым, виджет и часы — нет.
    assert.match(displayScript, /setVar\('--timer-color', timerColor\)/);
    assert.match(
        displayScript,
        /const setVar = \(name, value\) => \{[\s\S]{0,200}removeProperty\(name\)/,
        'у дисплея нет помощника, который СНИМАЕТ переменную цвета'
    );
