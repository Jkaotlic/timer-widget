...
test('пока сборки не подписаны, описание релиза об этом ГОВОРИТ', () => {
    ...
    if (unsignedMac || unsignedWin) {
        ...
        // Карантинная метка — второй, более грубый отказ macOS: система пишет
        // «приложение повреждено», хотя файл цел. Без этой строки пользователь
        // считает сборку битой и идёт удалять её, а не открывать.
        assert.match(notes, /com\.apple\.quarantine/,
            'нет команды снятия карантина: без неё «повреждено» читается как испорченный файл');
        assert.match(notes, /xattr -dr com\.apple\.quarantine/,
            'команда снятия карантина должна быть рабочей и рекурсивной (-dr)');
    } else {
