...
test('справка описывает интерфейс, который есть сейчас', () => {
    ...
    // Редизайн 2026-08-12 оставил четыре длительности и пятую ячейку «мин»
    // для своего времени: у неё НЕТ data-minutes, поэтому в этот список она
    // не попадает и не должна.
    const presets = [...HTML.matchAll(/class="preset" data-minutes="(\d+)"/g)].map((m) => +m[1]);
    assert.deepEqual(presets, [5, 15, 25, 45], 'состав кнопок пресетов изменился');
    assert.match(body, /Пресеты: 5, 15, 25, 45 мин/, 'справка не перечисляет реальные пресеты');
    assert.match(body, /Клавиши 1–4/, 'справка не упоминает клавиши 1–4');
...
test('мёртвые правила CSS удалены', () => {
    const dead = [
        ['.time-input', 'разметка использует .hero-input'],
