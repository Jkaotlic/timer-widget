...
test('у каждого окна СВОЯ роль моста, и все четыре разные', () => {
    ...
    assert.deepEqual([...roles].sort(), ['clock', 'control', 'display', 'widget']);
});
...
test('храповик SEC-12: фьюз file:// включён ровно пока жив перенос настроек', () => {
    ...
    //
    // Условие снятия (решено 26.09.2026): перенос держится ещё минимум ДВА
    // минорных релиза после выпустившего его, снимается в ТРЕТЬЕМ (выйдет в
    // 2.11 — живёт в 2.12 и 2.13, снятие в 2.14). Кто перепрыгнет все три,
    // потеряет настройки — прочитать file:// без фьюза нельзя.
