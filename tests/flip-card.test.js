...
test('створки строятся из КЛОНОВ цифры — стиль окна не переписывается', () => {
    ...
    try {
        ...
        // ЧЕТЫРЕ слоя, а не три. Четвёртый — статичный ВЕРХ с НОВОЙ цифрой,
        // тот самый, что открывается из-под падающей створки.
        //
        // Его не было, и это не замечалось, пока слои были прозрачны: роль
        // играла статичная цифра самой карточки, просвечивая сквозь стекло.
        // Как только слои сделали непрозрачными (14.08.2026), они закрыли её
        // собой, и из-под упавшей створки открывался ФОН карточки — жалоба
        // звучала как «при перелистывании верхняя часть цифры тёмная».
        assert.equal(wrap.children.length, 4, 'слоёв должно быть ровно четыре');

        const texts = wrap.children.map((c) => c.children[0].children[0].textContent);
        const classes = wrap.children.map((c) => c.className);
        // Порядок = порядок наложения снизу вверх.
        assert.deepEqual(texts, ['4', '3', '3', '4'],
            'снизу вверх: новый верх, старый низ, падающий старый верх, поднимающийся новый низ');
        assert.match(classes[0], /fc-top/, 'первым лежит статичный ВЕРХ');
        assert.match(classes[0], /fc-static/, 'верх статичен — он не анимируется, его открывают');
        assert.match(classes[1], /fc-bottom/, 'вторым — статичный НИЗ');
