...
// BUG-14: 100 ступеней по 0.29 ₽ — это 29 ₽, но 100 × 0.29 в двоичной
// арифметике даёт 28.999999999999996, и округление ВНИЗ при печати
// показывало залу «28 ₽». Цена округляется до копеек ДО того, как её режут.
test('BUG-14: дробная ставка не теряет рубль на двоичной погрешности', () => {
    assert.equal(Money.overrunCost(6000, 0.29, 60), 29);
    assert.equal(Money.formatMoney(Money.overrunCost(6000, 0.29, 60)), `29${Money.NBSP || ' '}₽`);
    assert.equal(Money.overrunCost(180, 0.1, 60), 0.3);
    // Округление вниз для ЦЕЛЫХ ступеней не тронуто: 59 с — ноль ступеней.
    assert.equal(Money.overrunCost(59, 0.29, 60), 0);
});

test('BUG-14: печать тоже снимает погрешность до округления вниз', () => {
    assert.equal(Money.formatMoney(28.999999999999996), '29 ₽');
    assert.equal(Money.formatMoney(28.99), '28 ₽', 'копейки по-прежнему отбрасываются');
});
