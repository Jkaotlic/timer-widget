...
test('полноэкранное окно называется одинаково везде', () => {
    ...
    // Редизайн 2026-08-12: имя окна живёт в СТРОКЕ списка, а data-tab висит
    // на шевроне рядом. Проверяем ту же связь по новой разметке — имя строки
    // и заголовок ящика обязаны совпадать.
    assert.match(PANEL_CODE, /<span class="wrow-name">Дисплей<\/span>[\s\S]{0,400}?data-tab="display"/);
...
test('карточки флипа — НЕПРОЗРАЧНЫЕ, без внешних теней и без backdrop-filter', () => {
    ...
    // До 14.08.2026 здесь проверялась ПОЛУПРОЗРАЧНОСТЬ (0.85, умноженная на
    // ползунок). Требование изменилось на противоположное, и не по вкусу:
    // карточка перекидыша — пластина, с которой створки берут свой фон
    // (цепочка `inherit` в flip-card.css), и сквозь полупрозрачную видно и
    // цифру под падающей створкой, и рабочий стол.
