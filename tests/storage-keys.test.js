...
function collect() {
    ...
    for (const file of runtimeFiles()) {
        ...
        // Третья форма: ключ передан модулю параметром. Так устроена геометрия
        // окон — window-geometry.js получает `storageKey` в настройках и сам
        // делает по нему и чтение (restore), и запись (save). Литерал остаётся
        // в разметке окна, но `localStorage.getItem('литерал')` рядом с ним не
        // стоит, и без этой ветки ключ выглядел бы «объявлен, но не используется».
        //
        // Засчитываем ОБА направления сразу, потому что модуль делает оба:
        // считать только чтением значило бы получить ложное «пишется, но не
        // читается» на соседнем окне.
        for (const m of src.matchAll(/\bstorageKey:\s*['"]([^'"]+)['"]/g)) {
            note(reads, m[1], file);
            note(writes, m[1], file);
        }
    }
    return { reads, writes };
}
