...
test('значение блока в «Цифрах» набирается ЦВЕТОМ ТЕКСТА, а не зелёным табло', () => {
    // Жалоба 26.08.2026: «почему в стиле „Цифры" функциональные блоки времени
    // зелёным цветом». Замер на светлом тоне: и подпись, и значение блока —
    // rgb(18,101,47), при том что сами цифры таймера rgb(29,29,31).
    //
    // Разбор развёл эти два случая:
    //   ПОДПИСЬ зелёная НАМЕРЕННО — у неё свой тон-зависимый токен
    //   `--style-led-label` (#5fe089 на тёмном, #12652f на светлом), и его
    //   контраст в обеих темах держит tests/contrast.test.js. Не трогаем.
    //   ЗНАЧЕНИЕ красилось `--tw-led-green` — остатком стиля LED, слитого с
    //   «Цифрами» 13.08.2026. Это общий акцент, а не стилевой токен; правило
    ...
    // тест падал бы на собственном тексте. Правило проекта, и оно сработало
    // на первом же прогоне этой проверки.
    const bare = css.replace(/\/\*[\s\S]*?\*\//g, '');

    const value = /body\.style-digits \.info-value \{[\s\S]*?\n\}/.exec(bare);
    assert.ok(value, 'не найдено правило значения блока в «Цифрах»');
    assert.match(value[0], /color: var\(--info-color, var\(--tw-fg\)\)/,
        'значение блока «Цифр» красится не цветом текста');
    assert.ok(!/--tw-led-green/.test(value[0]),
        'у значения блока «Цифр» вернулся зелёный LED');

    // Само-проверка зонда: он обязан НАХОДИТЬ --tw-led-green там, где тот
    // остаётся законно (плашка состояния). Иначе зелёный означал бы и
    // «убрано», и «регулярка ничего не ищет».
    assert.ok(/--tw-led-green/.test(bare), 'зонд не находит --tw-led-green нигде — проверка холостая');

    // Подпись обязана остаться на своём тон-зависимом токене.
    const label = /body\.style-digits \.info-label \{[\s\S]*?\n\}/.exec(bare);
    assert.ok(label, 'не найдено правило подписи блока в «Цифрах»');
    assert.match(label[0], /var\(--style-led-label\)/,
        'подпись «Цифр» потеряла свой токен — это решение осознанное, см. contrast.test.js');
});
