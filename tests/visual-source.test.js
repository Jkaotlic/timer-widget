...
/**
 * Оформление не переписывает ПОЛЬЗОВАТЕЛЬСКИЙ текст.
 *
 * Название мероприятия — единственное значение на дисплее, которое набирает
 * пользователь, и оно печаталось капслоком: `text-transform: uppercase` в
 * `.info-value-title`. «Планёрка отдела продаж» превращалась в «ПЛАНЁРКА
 * ОТДЕЛА ПРОДАЖ», а аббревиатуры и имена собственные внутри строки теряли
 * различие. Капслок остаётся возможным — его набирают, а не назначают стилем.
 *
 * Подписи, которые пишет САМО приложение («НАЧАЛО», «ОСТАЛОСЬ», плашка
 * состояния), прописными остаются намеренно: это его собственный текст.
 */
test('название мероприятия печатается как набрано, а подписи приложения — прописными', () => {
    const css = read('display.css');

    const titleRule = /\.info-value-title\s*\{([^}]*)\}/.exec(css);
    assert.ok(titleRule, 'правило .info-value-title не найдено');
    assert.doesNotMatch(
        titleRule[1],
        /text-transform\s*:\s*uppercase/,
        'название мероприятия снова печатается капслоком — оформление переписывает текст пользователя'
    );

    // Проверка отсутствия обязана видеть саму себя: та же регулярка на
    // подписи приложения должна СРАБОТАТЬ, иначе зелёный ничего не значит.
    const labelRule = /\.info-label\s*\{([^}]*)\}/.exec(css);
    assert.ok(labelRule, 'правило .info-label не найдено');
    assert.match(
        labelRule[1],
        /text-transform\s*:\s*uppercase/,
