...
test('свечений через text-shadow/drop-shadow не осталось', () => {
    // Это был ХРАПОВИК: этап A снял ореолы из box-shadow, а те же ореолы,
    // написанные через text-shadow и drop-shadow, жили в неоновых стилях LED
    // и «Цифры» трёх окон и снимались этапом D. Потолки стояли 25 / 10 / 6 и
    // могли только убывать.
    //
    // Этап D закрыт, все четыре файла на нуле — храповик превращён в запрет,
    // ровно как в нём и было записано. Мягкая нейтральная тень под текстом
    // сохраняется во всех окнах: она не красит, а отделяет цифры от фона,
    // который на виджете и дисплее задаёт пользователь.
    for (const file of SURFACE_FILES) {
        assert.equal(
            textGlowCount(read(file)), 0,
            `${file}: ореол вернулся через text-shadow или drop-shadow`
        );
    }
});
