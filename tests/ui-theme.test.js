...
test('окна с пользовательским фоном: виджет прибит темой, дисплей — яркостью', () => {
    ...
    // ВИДЖЕТ пин сохранил: он лежит поверх ЧУЖОГО рабочего стола и своего фона
    // не имеет вообще — измерять там нечего, палитра прибита темой.
    const widget = read('electron-widget.html');
    const widgetPin = /\[data-theme="light"\]\s*\{[\s\S]*?\}/.exec(widget);
    assert.ok(widgetPin, 'виджет: нет блока, закрепляющего палитру в светлой теме');
    assert.match(widgetPin[0], /--tw-fg:\s*#ffffff/, 'виджет: текст в светлой теме обязан остаться светлым');

    // ДИСПЛЕЙ после редизайна 2026-08-12 теме СЛЕДУЕТ, но цвет текста решает не
    // она, а яркость фактического фона: фон здесь задаёт пользователь, и тёмная
    // заливка при светлой теме дала бы чёрные цифры на чёрном. Поэтому вместо
    // пина по теме — две явные палитры по измеренной яркости.
    const display = read('display.css');
    assert.match(
        display, /:root:not\(\.on-light-bg\)[^{]*\{[\s\S]*?--tw-fg:\s*#ffffff/,
        'дисплей: на тёмном фоне текст обязан быть светлым'
    );
    assert.match(
        display, /:root\.on-light-bg\s*\{[\s\S]*?--tw-fg:\s*#1d1d1f/,
        'дисплей: на светлом фоне текст обязан быть тёмным'
    );
    assert.doesNotMatch(
        display, /\[data-theme="light"\]\s*\{[\s\S]*?--tw-fg:\s*#ffffff/,
        'дисплей: пин по ТЕМЕ вернулся — решать обязана яркость фона'
    );
