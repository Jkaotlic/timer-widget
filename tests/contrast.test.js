...
test('.bg-mode-btn.active — предсуществующий дефект контраста, ЗАФИКСИРОВАН числом, не исправляется здесь', () => {
    ...
    const rule = /^\.bg-mode-btn\.active[^{}]*\{([^}]*)\}/m.exec(control);
...
test('подпись блока — ОДИН тон на все четыре стиля, у «Цифр» своего нет', () => {
    // Здесь стояла обратная проверка: подпись «Цифр» обязана читать
    // --style-led-label, зелёный цвет табло. 26.08.2026 решение отменено по
    // просьбе «зачем надписи остались зелёными, в других стилях они белые».
    //
    // Обоснование зелёного было «цвет табло», и оно рассыпалось: стиль LED
    // слит с «Цифрами» 13.08.2026, сами цифры таймера с тех пор набраны
    // обычным цветом текста, а подпись — один и тот же элемент во всех
    // стилях. Замер на светлом тоне: «Круг», «Флип» и «Аналог» давали
    // rgb(49,49,54), «Цифры» — rgb(18,101,47). Выделяться было нечем.
    //
    ...
    assert.ok(
        ...
        'у «Цифр» снова появилась своя подпись — тон подписи должен быть один на все стили'
    );
    // Само-проверка зонда: перебивки ДРУГИХ свойств у «Цифр» есть, и регулярка
    // того же вида их находит. Иначе зелёный означал бы и «перебивки нет», и
    // «регулярка не работает».
    assert.ok(
        /body\.style-digits \.info-value \{/.test(bare),
        'зонд не находит даже правило значения «Цифр» — проверка холостая'
    );
