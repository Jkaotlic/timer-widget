...
test('окно управления больше не god-файл', () => {
    ...
    //
    // Поднят с 2970 до 2990 11.08.2026: панель получила режим полосы — 12
    // строк разметки самой полосы, кнопка-шеврон в титлбаре, тег <script> и
    // ДВЕ строки проводки (комментарий и вызов). Вся логика режима — 90 строк —
    // уехала в mini-bar.js, и именно этот потолок её туда отправил: к началу
    // работы в файле оставалось три строки запаса. Потолок inline-скрипта
    // снова НЕ поднят.
    assert.ok(lines < 2990, `electron-control.html разросся до ${lines} строк`);
});
