...
| `hero-modes.test.js` | Реестр режимов, границы суток, прошедшая отметка, конец раньше начала, потолок подписи |
| `hero-modes-wiring.test.js` | Герой берётся из ОДНОГО места — по ВЫЗОВАМ, не только телам; подпись имеет одного владельца; деньги читают сырой остаток; зонд проверяет себя |
...
- **Режим героя влияет на цвет ТОЛЬКО через тотал: `timerColorBand` сам даёт `overtime` при минусе и `normal` при нулевом тотале — «полосы от тотала», «только минус» и «полос нет» это один вход, а не три правила** — [разбор](docs/lessons.md#a-single-owners-test-must-cover-its-call-sites-not-its-body)
