...
- **High contrast is a surface ladder, not "the dark theme with the colour drained" (CRITICAL)**: `hc-dark` used to paint window, panels and controls all `#000000`, leaving borders as the only separation — and half the panel's fills are literals in `control.css` (`rgba(255,255,255,0.04–0.06)`) that no token can reach: visible over dark blue glass, invisible over pure black. The result read as a flat grey field. The theme now defines three SOLID steps (`--tw-level-1..3`, 18.3 / 15.8 / 12.5:1 under white text, each ≥1.6× the luminance of the one below) and `control.css` re-fills the controls inside a `[data-theme="hc-dark"]` block — theme-scoped, so the default theme is provably untouched (its capture frames stay byte-identical). Two rules are load-bearing: **text on an accent fill is `--tw-on-accent` (black)** — white on the lightened green is 1.9:1, which was the start button, the active preset and every toggle knob; and **disabled state is a step + border, never `opacity: 0.4`** — 40% of a dark fill on pure black is not "unavailable", it is "gone". `tests/contrast.test.js` holds the ladder and the accent rule at AAA; `e2e/ui-theme.spec.js` measures the COMPUTED colours of real controls, because a token that no rule applies proves nothing.
