 ...
 *
 * 18.08.2026 он его и поймал — на macOS и Windows в CI, при зелёном локальном
 * прогоне. Пин колонки исходил из «окно вырастет ровно на ширину ящика», а на
 * экране 1024 окно просило 1096 и получило 974: пин выходил на 122px шире
 * правды, панель прыгала вправо на 40px и возвращалась. Теперь пин считает
 * `PanelDrawer.drawerColumnWidth()` — та же арифметика, что у главного
 * процесса, включая обрезку по экрану. Ветку обрезки на широком мониторе не
 * воспроизвести в принципе, поэтому числа проверяет tests/panel-drawer.test.js.
 */
