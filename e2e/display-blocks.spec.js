...
test('подпись над таймером и плашка состояния: выключаются, тащатся и не двигают таймер', async () => {
    ...
    try {
        ...
        // Допуск в ПИКСЕЛЬ: полоса сверху (--top-band) пересчитывается по
        // ЗАМЕРУ карточек и округляется до целого, а сдвиг колонки — половина
        // полосы. Предмет проверки — что кольцо не поехало ЗА подписью
        // (дефект двигал его на десятки пикселей), а не что округление всегда
        // падает в ту же сторону.
        expect(
            Math.abs(g.ring.y - before.ring.y),
            `кольцо съехало: ${before.ring.y} → ${g.ring.y}`
