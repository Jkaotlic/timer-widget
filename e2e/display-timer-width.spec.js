...
test('размер цифр — функция запрошенного процента, а не порядка посылок', async () => {
    ...
    try {
        ...
        const at = (pct) => seen.filter((s) => s.pct === pct);
        const widths = (pct) => at(pct).map((s) => s.w);

        // Само-проверка: 150 % обязаны отличаться от 100 % по-настоящему,
        // иначе совпадение внутри групп не значит ничего.
        const base = widths(100)[0];
        const grown = widths(150)[0];
        expect(Math.abs(grown - base),
            `150 % и 100 % дали почти одно и то же (${base}px против ${grown}px) — спека холостая`)
            .toBeGreaterThan(50);
        ...
        expect(grown / base, `150 % дали ${grown}px при 100 % = ${base}px`).toBeCloseTo(1.5, 1);
    } finally {
