...
test.describe('режимы центрального времени', () => {
    ...
    test('«до конца» на прошедшем конце показывает минус и красный', async () => {
        ...
        try {
            ...
            // Отметки — ОТНОСИТЕЛЬНО часов окна: с пониманием полуночи (BUG-08)
            // «00:00–00:01» перестало быть «заведомо прошедшим» — после шести
            // часов льготы экран честно считает до завтрашнего. Час назад —
            // начало, две минуты назад — конец: прошедший конец в любое время
            // суток, включая переход через полночь (clockOf берёт по модулю).
            const t0 = await nowSecondsIn(display);
            await setEvent(control, clockOf(t0 - 3600 + 86400), clockOf(t0 - 120 + 86400));
            await pickMode(control, 'to-end');
    ...
    test('«Цифры» с заведомо двузначным часом не вылезают за рамку', async () => {
        ...
        // Арифметика ниже не зависит от того, какие сутки выберет
        // eventClockDistances (BUG-08): любой её ответ сравним с Δ по модулю
        // суток, а
    ...
    test('отметка проходит САМА — и круг краснеет от одних секунд', async () => {
        ...
        try {
            ...
            const t0 = await nowSecondsIn(display);
            // Отметка «через минуту» в последние минуты суток — уже завтрашняя,
            // и переход через полночь (BUG-08) здесь не то, что меряется.
            // Пропуск громкий и с причиной — честнее, чем тихо померить не
            // тот переход.
