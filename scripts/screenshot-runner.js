...
async function run({ app, log, ctx, applyTimerState, openWidget, openClock, openDisplay, outDir, nativeImage }) {
    ...
    try {
        ...
        const CANONICAL_SIZES = {
            // 740, а не 700: столько панель занимает СОДЕРЖИМЫМ, и столько же
            // ей даёт главный процесс (CONTROL_WINDOW_MAX_HEIGHT). Кадр 700
            // обрезал верх — то есть сверка сторожила окно, которого у
            // пользователя не бывает (19.08.2026, после появления ряда
            // пресетов вида).
            control: [400, CONFIG.CONTROL_WINDOW_MAX_HEIGHT],
            widget: [250, 250],
            clock: [250, 250],
            display: [1280, 720]
        };
