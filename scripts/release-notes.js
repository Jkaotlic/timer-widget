...
**Linux.** Поставляется только \`.deb\`: песочница Chromium работает всегда. На
Ubuntu 24.04+ пакет ставит профиль AppArmor для user namespaces, а SUID-бит на
вспомогательный бинарник выставляется лишь там, где ядро закрывает user
namespaces обычным пользователям.
Сборки с \`--no-sandbox\` нет ни одной: AppImage снят с поставки, потому что
без шага установки песочницу в нём не поднять.
...
- **macOS** — при первом запуске «не удаётся проверить разработчика»: откройте
  ...
  Если вместо этого система пишет, что приложение **«повреждено и не может быть
  открыто»**, дело не в файле: так macOS отвечает на карантинную метку у
  неподписанной сборки. Метка снимается одной командой — укажите свой путь к
  приложению:

  \`\`\`sh
  xattr -dr com.apple.quarantine /Applications/TimerWidget.app
  \`\`\`

  Если в ответ «Operation not permitted» — та же команда через \`sudo\`. Сумму
  файла перед этим стоит сверить со списком ниже: снимая карантин, вы берёте
  проверку подлинности на себя.
- **Linux** — \`sudo apt install ./TimerWidget-*.deb\`.
