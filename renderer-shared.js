...
// signedSecondsUntilClock(nowSeconds, 'HH:MM') → знаковое расстояние до отметки сегодня
// ---------------------------------------------------------------------------
/**
 * ЗНАКОВОЕ расстояние от системных часов до отметки: минус означает, что
 * отметка прошла.
 *
 * Разбор входа живёт в `clockToSeconds()`, в одном месте: отсюда и из
 * `eventClockDistances()` ниже. Две копии регулярки и диапазонов разошлись бы
 * на первом же исправлении.
 *
 * Точная формулировка: через `clockToSeconds()` идёт ВСЁ НОВОЕ, что разбирает
 * `'HH:MM'`, — но «единственный в проекте» сказать нельзя. `updateStaticMiniClock()`
 * в display-script.js разбирает ту же строку своими `split(':')` + `parseInt`;
 * он предсуществующий, у него другой потребитель (стрелки мини-циферблата на
 * плашках) и своя ветка. Сведение их — отдельная работа; писать здесь, что
 * копий нет, было бы неправдой.
 *
...
/**
 * Сколько осталось до конца мероприятия — для блока «До завершения» на
 * дисплее.
 *
 ...
 *
 * Формула — `eventClockDistances().toEnd`, та же, что у героя «До конца»:
 * какие это сутки, решает мероприятие целиком (начало И конец). До 26.09.2026
 * блок считал по одной отметке с клампом (`secondsUntilClock`) и в 23:30 при
 * мероприятии 22:00–00:10 показывал 00:00:00 — конец «сегодня в 00:10» уже
 * позади. Своё у блока только одно: он не показывает минус, прошедший конец —
 * это ноль («закончилось»); минус перерасхода — дело героя.
 *
 * @param {number} nowSeconds — секунды с начала суток
 * @param {string} startClock — 'HH:MM', начало мероприятия
 * @param {string} endClock — 'HH:MM', конец мероприятия
 * @returns {number} секунды, не меньше нуля
 */
function secondsUntilEventEnd(nowSeconds, startClock, endClock) {
    return Math.max(0, eventClockDistances(nowSeconds, startClock, endClock).toEnd);
}
