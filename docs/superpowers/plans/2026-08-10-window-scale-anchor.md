...
Expected: FAIL в обоих тестах. У виджета — на правом крае (`bounds.x + bounds.width` порядка 4170 против 3440), у часов — на нижнем (порядка 1940 против 1350). **Если тест зелёный сразу — остановиться и разобраться:** значит либо рабочая область машины больше, либо окно открылось не там, где ожидалось; в этом случае надо задать окну исходную позицию у края явно через `win.setPosition`, иначе тест ничего не доказывает.
