...
(адрес схемы `app://timer-widget/`, SEC-12; окно на `file://` — чужое);
остальное отбрасывается с одной записью в журнал на канал. Перенос настроек
file:// → app:// нового канала не заводит: у его скрытого окна нет preload,
данные забирает и отдаёт главный процесс через `executeJavaScript`. Новый канал без
