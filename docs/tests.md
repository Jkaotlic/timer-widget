...
| `e2e-profile-guard.test.js` | Сторож общего e2e-профиля: разница по полям, умолчания из таблиц настроек, доли вместо пикселей в местах карточек, изменчивые ключи с причиной и из реестра, сводная спека последняя; утечку сторож ВИДИТ |
...
| `release-gates.test.js` | DevTools guarded on EVERY window, isolation, no external URLs, local fonts, no auto-update, CSP per window, Linux: deb only, AppArmor `userns` + SUID as fallback, `--no-sandbox` in no target; navigation guard wired; `build.electronFuses` config; SEC-12 ratchet — fuse `grantFileProtocolExtraPrivileges` is `true` exactly while `main-storage-migration.js` exists; the one bridgeless window (migration) allowed only hidden with `devTools: false` |
...
| `navigation-guard.test.js` | SEC-06: навигация окна — только на четыре свои страницы схемы `app://timer-widget/` (хеш/query не мешают, `%2e%2e` не обходит; любой `file://`, чужой хост/порт — отказ); `window.open` и `<webview>` — отказ на каждом событии |
| `app-scheme.test.js` | SEC-12: разбор адреса схемы `app://` — хост, порт, учётка, обход каталога в любой записи (`..`, `%2e%2e`, `%2f`, `%5c`, NUL, двойное кодирование), список из разметки, каталоги ассетов, MIME, заголовки; CSP заголовка = meta каждой страницы = `csp-guard` |
| `main-app-protocol.test.js` | SEC-12: ответ `protocol.handle` настоящими `Response` — тело байт в байт, CSP/nosniff, 403/404/405, HEAD, файл только из каталога приложения, отказ в журнал без пути |
| `main-storage-migration.test.js` | SEC-12: перенос localStorage `file://` → `app://` на подставке, исполняющей ТОТ ЖЕ код, что уходит в окно: не перезаписывает, метка после сброса на диск, повтор после квоты/таймаута/сбоя, gave-up, свежий профиль по снимку на старте, окно гасится после панели, журнал без ключей |
...
| `app-scheme.spec.js` | SEC-12: окна на `app://timer-widget/` (origin, безопасный контекст, общее хранилище окон); ответ схемы через `net.fetch` главного процесса — CSP и nosniff заголовками, MIME, 404 на главный процесс/preload/обход каталога, 403 на чужой хост |
| `storage-migration.spec.js` | SEC-12: «старая версия» (`e2e/fixtures/seed-file-storage.js`) сеет `file://`, новая переносит всё в `app://` — байт в байт (фон ~3 М символов, кириллица, U+2028), окна показывают настройки; второй запуск не повторяет; повтор без метки не перезаписывает новое; свежий профиль без окна переноса |
| `zz-profile-leaks.spec.js` | ПОСЛЕДНЯЯ спека: общий профиль после каждого теста вернулся к «было» (сторож `e2e/profile-guard.js` сверяет и возвращает на `app.close()`, сверка и между тестами); печатает, после каких тестов возвращал |
