...
| `event-report.test.js` | Отчёт CSV: шапка, строки, экранирование по RFC 4180, BOM и CRLF; итог из накопителя, а не сумма строк; стоимость от СЕКУНД |
...
| `event-export.spec.js` | Выгрузка ПО КЛИКУ доходит до файла на диске: диалог подменён в главном процессе, содержимое читается с диска; зонд проверяет себя |
