...
- Итог CI после `b612c0e`: unit, build, pack и linux-sandbox зелёные на всех
  платформах, e2e ubuntu зелёный, macOS и Windows — **176 из 177**, единственное
  падение `drawer-layout.spec.js:162` («панель дёрнулась: уехала до 60px и
  вернулась»). Оно же падало 14.08.2026, до этой работы; ubuntu тогда был
  зелёным — он и есть опорная точка «что нового».
...
3. `drawer-layout.spec.js` на macOS и Windows — унаследованное красное, CI из-за
   него не зеленеет целиком.
